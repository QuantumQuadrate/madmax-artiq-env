...
class TTL4SquareWave(EnvExperiment):
    def build(self):
        self.setattr_device("core")
        self.setattr_device("ttl4")
        self.setattr_device("entangler0")
        self.entangle_inputs = [
            self.get_device("ttl{}".format(i)) for i in range(0, 4)
        ]
    ...
    def init(self):
        ...
        self.ttl4.pulse(1.5 * aq_units.us)  # marker signal for observing timing
    ...
    def setup_entangler(
        ...
        for channel in range(num_outputs):
            self.entangler0.set_timing_mu(channel, out_start, out_stop)
        for channel in range(num_inputs):
            self.entangler0.set_timing_mu(channel + num_outputs, in_start, in_stop)

        # NOTE: must set enable, defaults to disabled. If not standalone, tries to sync
        # w/ slave (which isn't there) & doesn't start
        self.entangler0.set_config(enable=True, standalone=True)
        self.entangler0.set_cycle_length_mu(cycle_len)
        self.entangler0.set_patterns(pattern_list)
    ...
    def run_entangler(self, timeout_length: TInt32):
        ...
        with parallel:
            ...
            end_timestamp, reason = self.entangler0.run_mu(timeout_length)
        ...
        self.entangler0.set_config(enable=False)
    ...
    def check_entangler_status(self):
        ...
        delay(100 * aq_units.us)
        num_cycles = self.entangler0.get_ncycles()
        rtio_log("entangler", "#cycles:", num_cycles)
        delay(100 * aq_units.us)
        ntriggers = self.entangler0.get_ntriggers()
        rtio_log("entangler", "#triggers (0 if no ref)", ntriggers)
        for channel in range(num_inputs):
            delay(150 * aq_units.us)
            channel_timestamp = self.entangler0.get_timestamp_mu(channel)
