...
class PeripheralManager(artiq.frontend.artiq_ddb_template.PeripheralManager):
    ...
    def _ttl_name(self, index):
        return f"ttl{index}"

    def _emit_edge_counter_named(self, name, channel):
        self.gen("""
    ...
    def process_entangler(self, rtio_offset, peripheral):
        ...
        # Physical DIO labels are ttl0..ttl(N-1) for inputs followed by outputs,
        # but the gateware exposes the outputs first in RTIO channel order.
        for output_index in range(num_outputs):
            self._emit_ttl_out(self._ttl_name(num_inputs + output_index), channel)
            channel += 1

        for input_index in range(num_inputs):
            name = self._ttl_name(input_index)
            self._emit_ttl_inout(name, channel)
            channel += 1
            if edge_counter:
                self._emit_edge_counter_named(name, channel)
                channel += 1
