"""SPCM1 single-click loopback test.

Wire ``entangler_output0`` to ``entangler_input1`` before running. Output 0 is
used as a fake photon pulse inside the SPCM gate. A successful run should report
outcome 2 and a nonzero click timestamp.
"""

from artiq.experiment import *


class AtomPhotonParity6SPCM1Loopback(EnvExperiment):
    def build(self):
